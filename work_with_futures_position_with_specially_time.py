    ...
    def obxod(self, coins):
        ...
        for i in range(len(self.users)):
            ...
            if self.sides[i] != -1:
                ...
                if self.sides[i] == 0:
                    self.take_price[i] = self.users[i].get_open_orders()[self.long[i]]['average_price'] * (100 + self.takes[i] / self.leverage[i]) / 100 + (self.users[i].get_open_orders()[self.long[i]]['average_price'] - self.users[i].get_open_orders()[self.long[i]]['last_price']) / (self.users[i].get_open_orders()[self.long[i]]['volume']/self.users[i].get_open_orders()[self.short[i]]['volume'])*1.5
                    # self.stop_price[i] = self.layers[i] * (100 - self.stops[i] / self.leverage) / 100
                else:
                    self.take_price[i] =self.users[i].get_open_orders()[self.short[i]]['average_price'] * (100 - self.takes[i] / self.leverage[i]) / 100 + (self.users[i].get_open_orders()[self.short[i]]['average_price'] - self.users[i].get_open_orders()[self.short[i]]['last_price']) / (self.users[i].get_open_orders()[self.short[i]]['volume']/self.users[i].get_open_orders()[self.long[i]]['volume'])*1.5
                    # self.stop_price[i] = self.layers[i] * (100 + self.stops[i] / self.leverage) / 100
            ...
            if self.strateges[i].col_orders > 6 and self.stop_price[i] == None:
                ...
                if self.sides[i] == 0:
                    ...
                    self.stop_price[i] = self.users[i].get_open_orders()[self.short[i]]['last_price'] * (
                                100 + self.stops[i] / self.leverage[i]) / 100
                orders.add_take_or_stop("set stop = " + str(self.stop_price[i]) + " side: " + str(self.sides[i]) + " last_price:" + str(self.users[i].get_open_orders()[0]['last_price']) + " first_price" + str(self.layers[i]) + " take:" + str(self.take_price[i]))
        self.delete()
        self.add()
        self.need_adds = []
        self.need_to_delete = []
    ...
    def in_need_to_close_take_or_stop(self, i, price):
        ...
        if self.sides[i] == 0:
            if price >= self.take_price[i]:
                ...
                print("GOOD take long", self.strateges[i].col_orders, time.time(), self.take_price, self.users[i].get_open_orders()[self.long[i]]['last_price'])
                return True
            if self.stop_price[i] != None and price < self.stop_price[i]:
                orders.add_take_or_stop("BAD stop long " + str(self.strateges[i].col_orders)+" " + str(self.users[i].get_open_orders(price)))
                print("BAD but STOP", time.time())
                return True
        if self.sides[i] == 1:
            if self.stop_price[i] != None and price > self.stop_price[i]:
                print("BAD but STOP short", time.time())
                orders.add_take_or_stop("BAD stop short " + str(self.strateges[i].col_orders)+" "+ str(self.users[i].get_open_orders(price)))
                return True
            if price <= self.take_price[i]:
                print(self.users[i].get_open_orders(price))
                orders.add_take_or_stop("GOOD take short")
                print("GOOD take short", self.strateges[i].col_orders, time.time(), self.take_price, self.users[i].get_open_orders()[self.short[i]]['last_price'])
