...
class strategy:
    def __init__(self, step_buy, step_sell, delta_time, multiplicity, unnormal_price_move, deposit, coin):
        ...
        self.unnormal_price_move = [200, 200]
        self.time = time.time()
        ...
        self.volumes_dokup_glav = [0.1, 0.2, 0.4,0.8,1.6,3.2,6.4,12.8]
        self.volumes_dokup_hedge = [0.1, 0.1, 0.2, 0.4, 0.8, 1.6, 3.2, 6.4]
    ...
    def what_to_do_normal(self, order_info, orders_info, price, k):
        ...
        if self.col_orders > len(self.volumes_dokup_glav):
            return [[0]]
        if self.col_orders == len(self.volumes_dokup_glav):
            buf = calc_last_order(orders_info[self.side_glav]['price'], orders_info[self.side_glav]['volume'],
                                  orders_info[1 - self.side_glav]['price'], orders_info[1 - self.side_glav]['volume'],
                                  price)
