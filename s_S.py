...
print(time.time())
# m2 = export_from_file("data_MATICUSDT_6000.csv")[:]
m2 = export(1, "BNBUSDT", 20)
print(m2[-1], m2[0])
m = [i[0] for i in m2]
# from work_with_third import *
...
leverage = 1
mas_1 = [5, 5, 8, 8, 10, 10, 10]
mas_2 = mas_1[::]
clas.add_user("A7hKrcISyhFZdzddCZ", "hBmMLMprHruxZL6QxTy35JpWsbBAR9234aWI", "ETCUSDT", leverage,
              [i * leverage for i in mas_1], [i * leverage for i in mas_2], 2,
              10000 * leverage,
              60 * 60, 15 * leverage, "marker", 5 * leverage, 500 * leverage, False, {"ETCUSDT": float(m[0])})
# time.add_time(3601)
...
for i in range(len(m)):
    ...
    # print(clas.users[-1].get_open_orders(price=  float(m[i])))
    time.add_time(60)
    col += 1
